...
void GrobImage::draw(QPainter *painter)
{
    if (image)
    {
        if (srcx >= 0)
        {
            if (dsth != 0 || dstw != 0)
            {
                painter->drawPixmap(QRectF(x,y,dstw,dsth), *image, QRect(srcx, srcy, srcw, srch));
            }
            else
            {
                painter->drawPixmap(x, y, *image, srcx, srcy, srcw, srch);
            }
        }
        else
        {
            if (dsth != 0 || dstw != 0)
            {
                painter->drawPixmap(QRectF(x,y,dstw,dsth), *image, image->rect());
            }
            else
            {
